    ...
    if request.method == 'POST':
        ...
        if conceptForm.validate():
            ...
            if conceptForm.saveDefinition.data:
                ...
                conceptCodes = conceptForm.codes.data
                posTerms = conceptForm.positiveTerms.data
                negTerms = conceptForm.negativeTerms.data
                codeFormat = conceptForm.codeFormats.data
                if conceptForm.updateDefinition.data:
                    # The concept definition and tree visual need updating.
                    response["action"] = "update"
                    task = tasks.concept_update.apply_async(args=[conceptCodes, posTerms, negTerms, codeFormat])
                    response["pollURL"] = url_for("codesFromConcepts.update_task_status", taskID=task.id)
                elif conceptForm.extractCodes.data:
                    # Extract the codes for the concept.
                    response["action"] = "extract"
                    task = tasks.code_extraction.apply_async(args=[conceptCodes, posTerms, negTerms, codeFormat])
